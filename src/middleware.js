...
export async function middleware( request ) {
  ...
  if(isConnected) {
    ...
    if (restrictedPaths.includes(request.nextUrl.pathname)) {
      return NextResponse.redirect((process.env.NODE_ENV === 'production' ? "https://" : "") + process.env.NEXT_PUBLIC_VERCEL_URL + '/dashboard');
    }
  }else{
    const restrictedPaths = ['/dashboard', '/profil'];
    if (restrictedPaths.includes(request.nextUrl.pathname)) {
      return Response.redirect((process.env.NODE_ENV === 'production' ? "https://" : "") + process.env.NEXT_PUBLIC_VERCEL_URL + '/login');
    }
  }
